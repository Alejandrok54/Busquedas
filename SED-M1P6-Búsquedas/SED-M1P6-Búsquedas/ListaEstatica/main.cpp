...
template<class T>
int Cola<T>::busquedaL(Constancia& c) const
    {
        for(int i=0;i<=ult;i++){
            if(datos[i] == c){
                return i;
...
int main() {
    ...
    do{
        ...
        case 2:
            if(MiCola.vacia()){
                std::cout<<"No hay constancias por elaborar.... De de alta alguna por favor\n";
            }else{
                x = MiCola.Front();
                MiCola.Dequeue();
                std::cout<<"***********Constancia**********\nDatos del alumno: \n";
        ...
        case 3:
            std::cin>>x;
            p = MiCola.busquedaL(x);
            if(p >= 0){
                std::cout<<"Su constancia se encuentra esperando en la posicion "<<p+1<<", por favor espere.....\n";
            }else{
                std::cout<<"Su constancia no se encuentra en la cola, cree una por favor.....\n";
            }
        }
    }while(op!=4);
